...
class OllamaBaseProvider:
    @staticmethod
    async def _get_installed_models(host: str | None = None) -> list[str]:
        """
...
class OllamaProvider(Provider, OllamaBaseProvider):
    def __init__(
        ...
        api_base_url: str | None = settings.LLM_BASE_URL,
...
class OllamaEmbeddingProvider(EmbeddingProvider, OllamaBaseProvider):
    def __init__(
        ...
        api_base_url: str | None = settings.EMBEDDING_BASE_URL,
    ):
